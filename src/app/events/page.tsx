...
const PAST = [
  { id: 'p1', date: 'Nov 2024', title: 'MoroccoAI Annual Conference 2024', location: 'Casablanca', type: 'Conference', attendees: '900+', href: '/conference' },
  { id: 'p2', date: 'Jun 2024', title: 'AI for Sustainable Development Workshop', location: 'Rabat', type: 'Workshop', attendees: '120', href: '#' },
  { id: 'p3', date: 'Mar 2024', title: 'Casablanca AI Meetup #8', location: 'Casablanca', type: 'Community', attendees: '80', href: '#' },
  { id: 'p4', date: 'Nov 2023', title: 'MoroccoAI Annual Conference 2023', location: 'Casablanca', type: 'Conference', attendees: '700+', href: '/conference' },
  { id: 'p5', date: 'May 2023', title: 'Hackathon: AI for Education', location: 'Online', type: 'Hackathon', attendees: '200+', href: '#' },
  { id: 'p6', date: 'Nov 2022', title: 'MoroccoAI Annual Conference 2022', location: 'Casablanca', type: 'Conference', attendees: '500+', href: '/conference' },
]

const TYPE_ACCENT: Record<string, string> = {
  Conference: '#C8202F', Webinar: '#1F2A5C', Community: '#F5F1EA',
  Workshop: '#1F2A5C', Hackathon: '#C8202F',
}
              ...
              <FadeUp key={ev.id} delay={i * 0.08}>
                <Link href={ev.href}
                  className={`group relative flex flex-col sm:flex-row sm:items-center gap-6 p-6 sm:p-8 rounded-2xl border transition-all duration-500 overflow-hidden ${
                    ev.featured ? 'border-red/25 bg-red/[0.02] hover:border-red/45' : 'border-bone/[0.06] hover:border-bone/14'
                  }`}
                >
                  ...
                  <div className="shrink-0 flex flex-row sm:flex-col items-baseline sm:items-start gap-3 sm:gap-0 sm:w-20">
                    ...
                    <div className="flex sm:flex-col gap-1.5 sm:gap-0">
                      <span className="text-label text-bone/38 text-[0.6rem]">{ev.date.month}</span>
                      <span className="text-label text-bone/22 text-[0.6rem] sm:mt-0.5">{ev.date.year}</span>
                    </div>
                  </div>
                  ...
                  <div className="flex-1 min-w-0">
                    <span className="text-label text-[0.58rem] mb-2 block"
                      style={{ color: TYPE_ACCENT[ev.type], opacity: ev.type === 'Community' ? 0.55 : 0.85 }}>
                      {ev.type}
                    </span>
          ...
          <div className="space-y-2">
            {PAST.map((ev, i) => (
              <FadeUp key={ev.id} delay={i * 0.05}>
                <Link href={ev.href}
                  className="group flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-8 py-5 border-b border-bone/[0.06] hover:border-bone/12 transition-colors duration-300">
                  <span className="text-bone/22 text-xs font-medium tabular-nums shrink-0 w-24">{ev.date}</span>
                  <span className="text-label text-[0.58rem] shrink-0"
                    style={{ color: TYPE_ACCENT[ev.type], opacity: 0.75 }}>{ev.type}</span>
                  <span className="flex-1 text-bone/65 text-sm font-medium group-hover:text-bone/90 transition-colors">{ev.title}</span>
                  <span className="text-bone/22 text-xs shrink-0">{ev.location}</span>
                  <span className="text-bone/18 text-xs shrink-0 hidden lg:block">{ev.attendees} attendees</span>
                </Link>
              </FadeUp>
            ))}
          </div>
